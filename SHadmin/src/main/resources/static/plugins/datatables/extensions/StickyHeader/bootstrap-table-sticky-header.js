...
(function ($) {
    ...
    BootstrapTable.prototype.initHeader = function () {
        ...
        function render_sticky_header(event) {
            ...
            // get header height
            var header_height = '0', nav_height = $('nav.navbar').height() | 0;
            if (that.options.stickyHeaderOffsetY) header_height = that.options.stickyHeaderOffsetY.replace('px','');
            // window scroll top
            var t = $(window).scrollTop();
            // top anchor scroll position, minus header height
            var e = $("#"+anchor_begin_id).offset().top - header_height - nav_height;
            // bottom anchor scroll position, minus header height, minus sticky height
            var e_end = $("#"+anchor_end_id).offset().top - header_height - nav_height - $('#'+table_header_id).css('height').replace('px','');
            ...
            if (t > e && t <= e_end) {
                ...
                $("#"+sticky_header_container_id).css('top', header_height + nav_height + 'px');
                // create scrollable container for header
                var scrollable_div = $('<div style="position:absolute;width:100%;overflow-x:hidden;left:-1px;" />');
